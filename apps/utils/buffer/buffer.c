...
void usage(char *bin){
	...
	fprintf(stderr, "useful if the output should go to one of the input files.\n\n");
	...
	fprintf(stderr, "if no files are specified, stdin is used for input, and stdout for output.\n");
	fprintf(stderr, "be forewarned that this has potential to use up a lot of memory.\n\n");
	fprintf(stderr, "NOTE: when redirecting output, your shell may open the target file for writing\nbefore the processes begin executing.  so using the \"-o\" option is safer than\nredirecting to a file.\n\n");
