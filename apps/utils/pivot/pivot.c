...
int pivot ( struct cmdargs *args, int argc, char *argv[], int optind ){
	...
	while ( fin != NULL ) {
		...
		while ( getline( &inbuf, &inbuf_sz, fin ) > 0 ) {
			...
			if ( n_keys )
				extract_fields_to_string( inbuf, keystr, keystr_sz, keys, n_keys, args->delim );

			/* make key string from pivots[] */
			extract_fields_to_string( inbuf, pivstr, pivstr_sz, pivots, n_pivots, args->delim );
			...
			for ( i = 0; i < n_values; i++ ) {
				tmplen = get_line_field( fieldbuf, inbuf, fieldbuf_sz - 1, values[i], args->delim );
				if ( tmplen > 0 ) {
		...
		for ( i = 0; i < n_key_strings; i++ ) {
			...
			for ( k = 0; k < n_pivot_keys; k++ ) {
				...
				if ( ! line_values )
					...
					for ( j = 0; j < n_values; j++ ) {
						printf("%.*f%s", value_precisions[j], line_values[j], j != n_values - 1 ? delim : "");
					}
				}
				if ( k != n_pivot_keys - 1 )
					fputs(delim, stdout);
