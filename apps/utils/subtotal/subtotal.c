...
int subtotal ( struct cmdargs *args, int argc, char *argv[], int optind ){
	...
	if ( getline(&inbuf, &inbuf_sz, in) > 0 ) {
		...
		keylen = get_line_field(prev_key_val, inbuf, SUBTOT_KEY_MAXLEN, key - 1, args->delim);
		fprintf(out, "%s", inbuf);

		/* prime the sums array with this line's values */
		for ( i = 0; i < nsums; i++ ) {
			if ( get_line_field(field, inbuf, SUBTOT_KEY_MAXLEN, sum_cols[i] - 1, args->delim) > 0 ) {
				sums[i] = atoi(field);
			}
		}
	}
	...
	while ( getline(&inbuf, &inbuf_sz, in) != -1 ) {
		...
		keylen = get_line_field(cur_key_val, inbuf, SUBTOT_KEY_MAXLEN, key - 1, args->delim);
